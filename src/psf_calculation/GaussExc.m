...
if length(rhofield)==1
    ...
    rhov = rhofield; 
    if length(zfield)>1
        zv = zfield(1) + (0:(zfield(2)-zfield(1))/dz)*dz;
    else
        zv = zfield;
    end
    if isempty(rhov)
...
elseif length(rhofield)==2
    ...
    dz = lamex/resolution(2);
    rhov = rhofield(1) + (0:(rhofield(2)-rhofield(1))/drho)*drho; 
    if length(zfield)>1
...
[~,~,tp0,ts0] = Fresnel(n0(1)*c0,[n0 n],d0);
...
if nargin>13 && ~isempty(atf)
    if length(atf)==1 % accounting for reflection losses when using water immersion; atf = ref index of cover slide
        ...
        [~, ~, tmp, tms] = Fresnel(n0(1)*c0,[n0(1) n0(1) atf(1)], -2*pi*atf(2)/lamex);        
        [~, ~, mp, ms] = Fresnel(sqrt(atf(1)^2-n0(1)^2*s0.^2),[atf(1) atf(1) n], 2*pi*atf(2)/lamex);        
    end
    tp0 = tmp.*mp.*tp0;
    ts0 = tms.*ms.*ts0;
else
...
tmpxt = zeros(numel(chi),2*maxm+1);
tmpxr = tmpxt;
tmpyt = tmpxt;
tmpyr = tmpxt;
tmpzt = tmpxt;
